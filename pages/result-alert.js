...
export default function ResultAlertPage() {
  ...
  const selectedOption = useMemo(() => {
    return allCurrentOptions.find((option) => optionKey(option) === selectedOptionKey);
  ...
  const filteredOptions = useMemo(() => {
    ...
    return allCurrentOptions.filter((option) => {
      const hay = `${optionText(option)} ${option?.formLabel || ""} ${option?.formKey || ""}`.toLowerCase();
          ...
          <div className="mb-6 flex items-center justify-between gap-3">
            ...
            <div className="hidden rounded-2xl border border-amber-200 bg-white/80 px-4 py-3 text-right shadow-sm sm:block">
              ...
              <p className="text-sm font-bold text-emerald-700">
                {source === "worker" ? "Live Dropdowns" : source === "loading" ? "Loading..." : "Fallback"}
              </p>
            </div>
          </div>
            ...
            <div className="rounded-[2rem] border border-white/80 bg-white/85 p-5 shadow-2xl shadow-amber-900/10 backdrop-blur sm:p-8">
              <div className="mb-6 rounded-3xl bg-slate-950 p-5 text-white shadow-lg">
                ...
                <p className="mt-3 text-sm leading-6 text-slate-200">
                  Course jis official result form me milega, system automatically wahi form use karega. B.Ed agar PG NEP page me hai, to wahi saved hoga.
                </p>
              </div>
                ...
                <label className="block">
                  ...
                  {selectedOption ? (
                    ...
                    <p className="mt-2 text-xs font-medium text-slate-500">
                      Exact official dropdown option select karo. System uska correct form URL save karega.
                    </p>
                  )}
                </label>
                ...
                <div className="space-y-3 rounded-3xl border border-slate-200 bg-slate-50 p-4">
                  <label className="flex gap-3">
                    ...
                    <span className="text-sm font-semibold leading-6 text-slate-700">
                      I agree that my result preview may be sent to admin Telegram group/channel for result alert handling.
                    </span>
                  </label>
                  ...
                  <label className="flex gap-3">
                    ...
                    />
                    <span className="text-sm font-semibold leading-6 text-slate-700">
                      I agree to receive PDUSU result summary on my personal WhatsApp number.
                    </span>
                  </label>
                </div>
                  ...
                  >
                    ...
                    {message.data?.trackingId ? (
                      <div className="mt-3 rounded-2xl bg-white/70 p-3 text-xs">
                        ...
                        <p className="break-all font-black">{message.data.formUrl}</p>
                      </div>
                    ) : null}
                  </div>
                ) : null}
            ...
            <aside className="space-y-5">
                ...
                <div className="mt-5 space-y-3">
                  <div className="rounded-2xl border border-slate-200 bg-white p-4">
                    ...
                    <p className="mt-1 text-sm leading-6 text-slate-600">
                      System Railway worker se official result forms ke dropdown options fetch karta hai.
                    </p>
                  </div>
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-4">
                    ...
                    <p className="mt-1 text-sm leading-6 text-slate-600">
                      Course jis official page me milta hai, वही form URL save hota hai.
                    </p>
                  </div>
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-4">
                    ...
                    <p className="mt-1 text-sm leading-6 text-slate-600">
                      Result found hone par admin Telegram preview aur student WhatsApp summary.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-[2rem] bg-amber-500 p-6 text-white shadow-xl shadow-amber-500/20">
                ...
                <p className="mt-3 text-sm leading-6 text-amber-50">
                  B.Ed/M.Ed/Annual/NEP results different forms me ho sakte hain. Isliye exact official option choose karna compulsory hai.
                </p>
              </div>
            </aside>
          </div>
