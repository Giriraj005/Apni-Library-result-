...
function shouldIncludeRegistration(reg, { courses, semesters, yearParts }) {
...
export async function createQueueForBatch({
  ...
  registrationsSnap.docs.forEach((doc) => {
    ...
    if (!shouldIncludeRegistration(reg, { courses, semesters, yearParts })) {
      return;
    }
    ...
    const formUrl = getFormUrlForYearPart(exactYearPart);
