...
export async function submitAspNetResultForm({
  ...
  const typeValue = findOptionValue(typeSelect, resultType);
  ...
  if (yearSelect?.name) {
    form.set(yearSelect.name, yearValue || yearPart);
  }

  if (typeSelect?.name) {
    form.set(typeSelect.name, typeValue || resultType);
  }
  ...
  if (submitName) {
    form.set(submitName, "Show Result");
  }
...
function findSelect(selects, keywords) {
  return (
    ...
    selects.find((s) =>
      s.options.some((o) =>
        /semester|main|reval|b\.a|b\.sc|b\.com/i.test(
          `${o.label} ${o.value}`
        )
      )
    )
...
function findOptionValue(select, label) {
  ...
  const exact = select.options.find(
    (o) =>
      cleanText(o.label).toLowerCase() === target ||
      cleanText(o.value).toLowerCase() === target
  );
  ...
  const loose = select.options.find((o) =>
    cleanText(`${o.label} ${o.value}`).toLowerCase().includes(target)
  );
