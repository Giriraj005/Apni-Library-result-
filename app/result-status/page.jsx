  ...
  async function check(e) {
    ...
    try {
      const res = await fetch(
        `/api/check-result-status?rollNo=${encodeURIComponent(rollNo)}`
      );
  ...
  return (
    <main className="container" style={{ padding: "36px 0" }}>
      <section className="card" style={{ padding: 24 }}>
        ...
        <p className="muted">
          Apna roll number enter karke registration aur result status check
          karein.
        </p>
          ...
          <div className="card" style={{ padding: 18, marginTop: 18 }}>
            {result.success ? (
              <>
                ...
                {result.output?.telegramSent && (
                  <p>Telegram group me result preview send ho chuki hai.</p>
                )}
              </>
            ) : (
              <>
